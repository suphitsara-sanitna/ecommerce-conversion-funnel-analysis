...
Progress bar and step indicator at checkout
Simplify the checkout experience


Dataset
This project uses the same dataset as the E-Commerce EDA Project — 200,000+ records across 7 tables covering transactions from an e-commerce business. Please refer to that repository to download the dataset.
...
SQL queries to extract and analyze event-level data tracking user behavior across the funnel
Segmentation analysis by device, traffic source, and country to identify whether drop-off patterns differ across user groups
A dashboard in Power BI to visualize funnel performance and revenue recovery opportunities
...
Send reminder emails and texts to users who abandon cart or checkout, targeting those who have opted in to marketing communications
Add a progress bar and step indicator throughout the checkout process to encourage completion
Investigate and simplify the checkout experience to reduce friction across all devices and traffic sources
...
Collect additional data at the checkout stage such as payment method, error logs, and shipping cost to better identify the root cause of drop-off
Run A/B test on checkout UX to identify which checkout experience leads to higher conversion
Build a deeper analysis on cart abandonment behavior once additional data is available
